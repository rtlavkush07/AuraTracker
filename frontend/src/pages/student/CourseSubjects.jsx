    ...
    useEffect(() => {
        if (userId && courseId) {
            const fetchSubjects = async () => {
                try {
                    const response = await axios.get(`/api/student/getCourseSubjects/${userId}/${courseId}`, {
                        headers: { Authorization: `Bearer ${token}` },
                    });
        ...
        <div className="flex flex-col gap-5 w-3/4">
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {loading ? (
                    ...
                    <div className="text-center text-white">No subjects available</div>
                ) : (
                    subjects.map((subject, index) => (
                        ...
                        >
                            <h3 className="text-lg font-semibold text-gray-600">{subject.name}</h3>
            ...
            {modalOpen && selectedSubject && (
                <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex justify-center items-center">
                    <div className="bg-white p-5 rounded-lg w-96">
                        <h2 className="m-0 text-xl">{selectedSubject.name}</h2>
                        <p className="my-2">Completed: {selectedSubject.progress || 0}%</p>
